...
The \textbf{aspiration} of this work is to explore and investigate certain aspects of the human computer interaction (HCI)\nomenclature{HCI}{Human Computer Interaction} related to natural user interfaces (NUIs)\nomenclature{NUI}{Natural User Interface}. Hence, the \textbf{hypothesis} presented by this project can be enunciated as follows:

\begin{quotation}
Modal redundancy is more precise, efficient and user friendly for artifact and scenario RST\nomenclature{RST}{Rotate, Scale and Translate} virtual object manipulation tasks than modal specialization on artifact and scenario respectively.
...
The main \textbf{purpose} of this project is to implement a user interface for the visualization of 3-D objects on a 55 inch-3-D\nomenclature{3-D}{Three-dimensional} capable screen with 1920x1080 resolution and 240Hz frame rate through a stereoscopic representation, and to enable their manipulation through the integration of the functionality of a multi-touch-overlay with unlimited touch point recognition and 200 FPS\nomenclature{FPS}{Frames Per Second} sample rate and a camera with a 512×424 CMOS \nomenclature{CMOS}{Complementary Metal Oxide Semiconductor} 3-D Time-of-Flight (ToF)\nomenclature{ToF}{Time of Flight} Image Sensor with Multi-Frequency Photo-Demodulation up to 130MHz and 2 GS/s \nomenclature{GS/s}{Gigasamples Per Second} ADC\nomenclature{ADC}{Analog to Digital Converter}.
...
In conformity with the definition proposed by Bourget \cite{Multimodal} from the Queen Mary University of London, multimodal interaction refers to interaction with the virtual and physical environment through several natural modes of communication. In the particular case of this project, it refers specifically to touch and free hand gestures.

\subsubsection {Modal redundancy}
Two or more input modalities are redundant within a user interface (UI)\nomenclature{UI}{User Interface} when the same type of visualization manipulation can be carried out by any of them indistinctly.
...
For the purposes of this work, a position is the spatial location of an object. All coordinate systems used in this project are right-handed.
...
\item
The user, using any of the three input methods \textbf{must} be able to translate the displayed object and the camera in all directions, giving 3 DOF for translation.
\item
...
\item
The program \textbf{should} offer the option to toggle full-screen modus.
\end{enumerate}
...
The implementation of gestures that combine several input types has been excluded to maintain the smoothness of the simulation and to avoid tracking inconsistencies between input devices. 

The position of the table and the referencing of hand and body positions relative to it has been intentionally left out to reduce error proneness and to allow the user to start the interaction anywhere.

An adequate calibration of table, world and camera space coordinates would be exceedingly complicated and time consuming and would require a very specific placement of the kinect camera. Besides, the interaction could only be triggered at very specific hitzones, which would reduce user friendliness significantly.

The current configuration of the software gives the user absolute freedom of choice for the position of the kinect camera. Furthermore, the table's default configuration is set to horizontal, but this can be easily adjusted for the use with a vertical screen by a minor modification of the code (See appendix \ref{app:codeVert}).
...
This subsection gives a superficial explanation of the hardware technology
\subsubsection{Stereography}
Stereography is a technique that works with parallax to generate two 2-D\nomenclature{2-D}{two-dimensional} images that can be processed by the brain into a single 3-D image by creating the illusion of depth.

For this project, the perception of 3-D is enabled through the use of active shutter 3-D glasses. These glasses synchronize with the 3-D screen and alternately block the sight of one eye by applying voltage to the liquid crystal layer and turning it opaque, so that only the image intended for the other eye is perceived.

\subsubsection{Touchscreen}
Touchscreens allow the user to interact with the software by tracking the position of one or many fingers and feeding the position information to the program.

The specific software used for this project uses an in-cell capacitive touch screen. The finger distorts the screen's electrostatic field and this disruption can be tracked and converted into a signal.

\subsubsection{Time Of Flight Camera}
ToF cameras can calculate the distance between the source and the target by measuring the travel time of an amplitude-modulated light at each pixel.

The kinect sensor, like other ToF cameras, offers a 1080p HD wide-angle camera with accurate high pixel resolution, low motion blur and high dinamic range depth image and ambient light invariant brightness image, also called active infrared (AIR)\nomenclature{AIR}{active infrared} for use in conditions of low light.
...
\subsubsection {Structure and Functions}
	\paragraph { File compatibility}
	...
	\paragraph { 3-D Visualization panel}
	...
	\paragraph { Mouse and Keyboard event handling}
...
The program is designed to give the user the impression of being able to grab the object or the scenario in the air and manipulate it in different ways, allowing for an intuitive translation of movements into visual feedback.

\subsection {Stereoscopy Implementation}
The first modification of the original program was the use of Quad-buffering to enable active stereoscopic display of the visualized objects. (See Appendix \ref{app:StereoFormat}). For this purpose, the introduction of parallax, achieved through a second camera, was necessary, which also implied the duplication of all homogeneous transformations and projections.

\subsection {New Keyboard Shortcuts}
According to Shneiderman\rq{}s 8 golden rules \cite{Shneiderman}, designers should cater to universal usability, allowing frequent users to expedite routine tasks and have better control over the interface.\footnote{The Full-screen function is also available on the top screen menu}
\begin{center}
...
Ctrl+F & Toggle Full-screen \\\hline
Ctrl+P & Screenshot\\\hline
Ctrl+M & Show Mouse\\\hline
Ctrl+H & Hide Mouse\\\hline
Ctrl+0 & Reset Artifact Matrix\\\hline
Ctrl+R & Reset View Matrix\\\hline
\end{tabular}
\end{center}

\subsection {Touch Screen Enhancements}
This subsection contains the gestures that SAVKiT can interpret from the touchscreen. The kind of manipulation depends on the number of fingers on the screen. 

When a first finger touches the screen, the touchscreen coordinates are mapped to the widget coordinates where the object is displayed. If the mapped coordinates are inside of the widget, the touchscreen interaction begins. 

Successive fingers activate other functions, and if a finger leaves the screen, the interaction adapts to the new quantity of fingers on the touch screen. If no fingers are left, the touch screen interaction comes to an end.

\subsubsection {One finger interaction}
If there is only one finger on the screen, then the user can change the position of the camera along the X and Y axes, giving the impression that the whole scenario is moving in the same direction as the finger on the screen. (See Figure \ref{fig:T1F}).
...
Let $F_{1,t}=\begin{pmatrix} x_{1,t}\\y_{1,t}\\ \end{pmatrix} \in \R^2, t\in \Z$ be the initial finger position at frame t with $(x,y)$ coordinates $(x_{1,t},y_{1,t})$ and let 
...
be the finger displacement. 

The translation vector $\begin{pmatrix}
F_{1,t}\\0
\end{pmatrix}\in \R^3 $ is then applied to the model matrix.

\subsubsection {Two-finger interaction}
If there are two fingers on the screen and the distance between them is increased or reduced, then the object is scaled correspondingly. If the line between the fingers is rotated, the object will rotate around the axis that is orthogonal to the screen plane. (See Figure \ref{fig:T2F}).

\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.39]{Images/Touch2Fingers.png}
...
where $\|\cdot \|_1$ is the $l_1$ norm, since its calculation is significantly more efficient than the euclidean distance. 

Then, the scaling operations are executed by the following formula:
$$\Mod_t=\frac{d_{t-1_{1,2}}}{d_{t_{1,2}}}\cdot \mathbb I\cdot\Mod_{t-1} $$
...
where $v=v_t$ and $u=v_{t-1}$

\subsubsection {Three-finger interaction}
If there are three fingers on the screen and they are moving in the same direction, a simulation of the trackball movement is initialized. Depending on the mode set for the trackball, the object follows a 3-D rotation as a sphere or in the plane. (See Figure \ref{fig:T3F}).

\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.70]{Images/Touch3Fingers.png}
\caption{Three Finger Touch Interaction}
\label{fig:T3F}
\end{figure}
\end{center}

Let $F_{i,t}$ be defined as above. Let n be the number of fingers touching the screen, in this case $n=3$, then the position of the pointer for the trackball movement is determined by:
$$\sum_{i=1}^n \frac{1}{n}F_{i,t}$$

\subsubsection {Four-finger interaction}
If there are four fingers on the screen and the distance between them is increased, then the distance from the camera to the object is reduced, if the distance between the fingers is reduced, then the camera goes further from the object, giving the impression of zooming in and out, respectively. (See Figure \ref{fig:T4F}).

\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.4]{Images/Touch4Fingers.png}
\caption{Four Finger Touch Interaction}
\label{fig:T4F}
\end{figure}
\end{center}

Let $F_{i,t}=\begin{pmatrix} x_{i,t}\\y_{i,t}\\ \end{pmatrix} \in \R^2, t\in \Z$ be the position of finger $i$ at frame t with $(x,y)$ coordinates $(x_{i,t},y_{i,t})$. The distance between fingers $j$ and $k$ at frame t $d_{t{{j,k}}}$ is calculated as follows:
$$d_{t_{j,k}}=\|F_{j,t}-F_{k,t}\|_1$$
...
$$CDO_t=CDO_{t-1}\cdot \frac{d_{t-1_{1,2}}+d_{t-1_{3,4}}}{d_{t_{1,2}}+d_{t_{3,4}}}$$

\subsubsection {Five-finger interaction}
If there are five fingers on the screen, then the user can change the position of the camera along the X and Y axes, giving the impression that the whole scenario is moving in the same direction as the fingers on the screen. (See Figure \ref{fig:T5F}).

\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.55]{Images/Touch5Fingers.png}
\caption{Five Finger Touch Interaction}
\label{fig:T5F}
\end{figure}
\end{center}

For the sake of simplicity, the program only considers the position of the last finger that touched the screen.
...
\subsubsection {Six-finger interaction}
If there are six fingers on the screen, the first five work as an anchor and the sixth one rotates the scenario around the vector orthogonal to the screen plane when moved around the average position of the other five fingers. (See Figure \ref{fig:T6F}).

\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.4]{Images/Touch6Fingers.png}
\caption{Six Finger Touch Interaction}
\label{fig:T6F}
\end{figure}
\end{center}
The center position of the first five fingers $F^{(5)}_t$ is calculated by
$$\sum_{i=1}^n \frac{1}{n}F_{i,t}$$
with $n=5$.
Afterwards, the program draws a line between this middle point and the position of the sixth finger and calculates the rotation angle of this line from frame to frame and rotates the view matrix accordingly.

Let $\overrightarrow{v_t}=F^{(5)}_t-F_{6,t}$ be the line between the sixth finger and the average position of the other 5 at frame t. Then, the angle of rotation $\theta_{touch}\in [0,360]$ around the vector orthogonal to the screen plane is calculated by: 
\[\theta_{touch}\frac{-180}{\pi}\cdot \arccos(<\overrightarrow{u}, \overrightarrow{v}>)\]
where $v=v_t$ and $u=v_{t-1}$.

\subsection {Kinect 2 Camera Gesture implementation}\label{KinectGestures}
Most current video games and applications for Kinect generate a pointer as an interaction dummy for the virtual world. SAVKiT uses absolute position data, which means that the user can start interaction anywhere in the kinect camera space and the movement will be calculated from the position of the hands on one frame relative to the next one.
...
Both hands closed start the scaling and rotation of the object, until one or both hands are opened. The object can be proportionally scaled by changing the separation between hands while keeping both hands closed. The rotation is programmed with 6 degrees of freedom (DOF).

If both hands have two fingers up, recognized as lasso position by the kinect, the program initializes the scenario rotating and scaling mode. The scenario can be proportionally scaled by changing the separation between hands while keeping the hand position and also rotated with 6 degrees of freedom
...
Once again, to ensure the smoothness of the scaling and to avoid abrupt changes, scaling factors outside of the tolerance borders are ignored, meaning that only if $0.8<s_t<1.25$, the scaling takes place.
...
The right hand up and open, the left one closed starts a homogeneous translation of the view matrix, where the left hand\rq{}s change of position determines the translation of the camera, and the left one works as the \lq\lq{}signal\rq\rq{}, meaning that the movement stays on as long as the right hand remains open, or until the left hand opens.

The translation vector $\Delta_l\in \R^3$ applied to the view Matrix is determined by:
$$\Delta_l=L_{t-1}-L_t, t\geq 1$$

Similar to the model translation, the scenario translation follows the movement direction of the hand.
...
Throughout the implementation of this software, several technical difficulties had to be dealt with. 

The main difficulty was the precarious, inconsistent and unintelligible documentation of the Touchscreen software. For this reason, the touch screen implementation had to be achieved through a trial and error method.

Also the finger tracking of the touchscreen represented an obstacle for a smooth implementation. However, this could be easily dealt with by programming internal methods that are largely independent of an adequate tracking.

Ambient light conditions also affected the kinect and touchscreen tracking, but this can be easily avoided by allocating the hardware away from direct sunlight.
%TODO: MORE HERE
...
\subsection {Vertical Screen Code}\label{app:codeVert}
\begin{lstlisting}
    if( !m_kLTr&&
    (m_kinectIsWatching&&
    (m_kLCC==2&&m_kROC>2))&&
...
//y and z coordinates are swapped
        m_kInitialTrPos= QVector4D(
        (qreal)-mano.x,
        (qreal)mano.z,
        (qreal)-mano.y, 1);
...
\subsection {Kinect Rotate Scale Code}\label{app:codeRot}
\begin{lstlisting}
//refresh hand positions
if(!mano.isLeft)
{
 m_kNewRightRotPos=QVector3D(
 (qreal)mano.x,
 (qreal)-mano.z,
 (qreal)mano.y);
 }
 ...
 * 180.0 / PI;
 ...
 m_MatrixArtefact.inverted().mapVector(m_axis) ); (*\label{lineDerotate}*)
 ...
 m_MatrixArtefact.rotate(
 m_kRotAngle,m_axis.normalized());
 if(m_kNewRotVec.length()/m_kOldRotVec.length()<1.25f 
 &&m_kNewRotVec.length()/m_kOldRotVec.length()>0.8f)
 m_MatrixArtefact.scale(
 m_kNewRotVec.length()/m_kOldRotVec.length());
 m_MatrixArtefact.translate(-m_camOrigin);
 m_MatrixArtefact.translate(-m_model->m_centerPoint);
 }
updateGL();
...
\begin{center}
Gender:M/F\ \ \ Age:\_\_\_ Occupation (Major for students):\_\_\_\_\_\_\_
\begin{enumerate}
...
\emph{One finger} on the screen activates the translation of the object. 
...
\emph{Two fingers} on the screen activate scaling and rotation of the object around the z-axis. both work simultaneously. Pinch to zoom and turn to rotate.
...
\emph{Three} fingers on the screen rotate the object in 3-D around its center. The 3-D rotation creates an imaginary line between the center of the three fingers and the object\rq{}s center and rotates the object accordingly. Turning the fingers has no effect.
See figure \ref{fig: AT3F}.
\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.7]{Images/Touch3Fingers.png}
\caption{Three Finger Touch Interaction}
\label{fig: AT3F}
\end{figure}
\end{center}
...
\subsubsection {Four-finger interaction}
If there are four fingers on the screen, the scenario can be zoomed in and out. (See Figure \ref{fig:AT4F}).
...
\subsubsection {Five-finger interaction}
With five fingers the scenario can be moved in x and y directions.(See Figure \ref{fig:AT5F}).
...
\subsubsection {Six-finger interaction}
If there are six fingers on the screen, the first five (hand) work as an anchor and the sixth one rotates the scenario.(See Figure \ref{fig:AT6F}).
...
\item Both hands closed start the object rotation and scaling.

\item Both hands with two fingers up (lasso) initialize the scenario rotating and scaling mode.
\end{itemize}
...
\item The object and scenario can be rotated in all directions.

\item Rotation stops when one hand is opened.

\item After this, to restart object rotation, open both hands and close them again.

\item To restart scenario rotation, open both hands and put them in lasso position again.
...
\caption{Kinect Closed Hands Movements}
\label{fig: AKR}
\end{figure}
\end{center}
...
\begin{itemize}
\item The object (and the scenario) can be enlarged (zoom in) by separating both hands and shrunk (zoom out) by nearing them.

\item Avoid moving both hands on the same direction to reduce flickering.

\item See figure \ref{fig: AKS}.

\end{itemize}
\begin{center}
\begin{figure}[!ht]
\includegraphics[scale=0.4]{Images/KinectScale.png}
\caption{Kinect Lasso Movements}
\label{fig: AKS}
\end{figure}
\end{center}
